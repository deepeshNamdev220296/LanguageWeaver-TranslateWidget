...
export function getAllTextNodes(
  ...
  if (skipAds(node)) return textNodes;
  traverse(node);
  return textNodes;
  ...
  function traverse(node: Node) {
    ...
    if (isTextNode(element)) {
      ...
      node.childNodes.forEach((subnode) => {
        traverse(subnode);
        hasText ||= isTextNode(subnode as HTMLElement);
      });
      if (hasText) {
...
function HandleFrameNode(node: HTMLElement) {
  if (node.nodeType === 1 && node.nodeName.toLowerCase() === "iframe") {
    let frameDoc = (node as HTMLIFrameElement).contentDocument;
...
function HandleOptionNode(node: HTMLElement) {
...
function IsTextString(text: string | null): boolean {
...
function CheckHasBeenTranslated(node: HTMLElement | null): boolean {
  if (node === null || node.parentElement === null) return true;
  const pn = node.parentElement!;
  return pn.nodeType !== 1 || pn.getAttribute("TranslatedAlready") !== null;
}

function MarkAsTranslated(node: HTMLElement | null): void {
  if (
    node === null ||
    node.nodeType !== 1 ||
...
function skipCodes(node: HTMLElement): boolean {
...
function skipAds(node: HTMLElement): boolean {
  if (
    !(
      node.nodeType === 1 &&
...
// export function getSelectedTextNodes(): HTMLElement[] {
// 	const selection = document.getSelection();
// 	console.log('[Selection] Current selection:', selection);

// 	if (!selection || selection.rangeCount === 0) return [];

// 	const selectedText = selection.toString().trim();
// 	console.log('[Selection] Selected text:', selectedText);

// 	if (!selectedText) return [];

// 	// Create a hash of the selected text to detect repeated triggers
// 	const selectionHash = `${selectedText}-${selection.anchorOffset}-${selection.focusOffset}`;
// 	if (selectionHash === lastSelectionHash) {
// 		console.log('[Selection] Same selection detected, skipping...');
// 		return [];
// 	}
...
// 		nodes.push(...getTextNodesInRange(range));
// 	}

// 	// Deduplicate by reference
// 	const uniqueNodes = Array.from(new Set(nodes));

// 	console.log('[Selection] Collected unique text nodes:', uniqueNodes);
// 	return uniqueNodes;
// }
export function getSelectedTextNodes(): HTMLElement[] {
  const selection = document.getSelection();
...
export function getTextNodesInRange(range: Range): HTMLElement[] {
  const nodes: HTMLElement[] = [];
  const ancestor = range.commonAncestorContainer;

  // If the ancestor is a text node and intersects the range, return it directly
  if (ancestor.nodeType === Node.TEXT_NODE && range.intersectsNode(ancestor)) {
    nodes.push(ancestor as HTMLElement);
    return nodes;
  }
